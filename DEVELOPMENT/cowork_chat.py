...
class CoworkChatApp:
    def __init__(self, root):
        ...
        # Geometry positioning: bottom-right corner
        self.width = 320
        self.height = 450
        screen_w = self.root.winfo_screenwidth()
        screen_h = self.root.winfo_screenheight()
        x = screen_w - self.width - 25
        y = screen_h - self.height - 75 # Account for Windows taskbar
        self.root.geometry(f"{self.width}x{self.height}+{x}+{y}")
        
        # Theme colors (Obsidian Dark / Synthwave Accent)
        self.bg_color = "#0e0e12"
        self.header_bg = "#161622"
        self.text_bg = "#1a1a24"
        self.fg_color = "#e2e8f0"
        self.accent_color = "#00d4ff" # Neon Cyan
        self.purple_color = "#bb86fc" # Neon Purple
        ...
        self.append_message("System", "Antigravity Cowork Chat has initialized. Speak or type instructions. Try 'switch to SHOGI' or 'run'.")

    def setup_ui(self):
        # Header bar (grabbable for dragging)
        self.header = tk.Frame(self.root, bg=self.header_bg, height=40)
        self.header.pack(fill="x", side="top")
        self.header.pack_propagate(False)
        
        # Window drag bindings
        ...
        # Brain Status bar
        self.status_bar = tk.Frame(self.root, bg="#1e1e2d", height=30)
        self.status_bar.pack(fill="x", side="top")
        self.status_bar.pack_propagate(False)
        
        self.status_lbl = tk.Label(self.status_bar, text="Brain: LOADING...", bg="#1e1e2d", fg=self.purple_color, font=("Segoe UI", 9, "bold"))
        ...
        self.trigger_btn.pack(side="right", padx=10)
        ...
        # Custom Tags for rich message colors
    ...
    def _api_send_chat(self, msg):
        try:
            ...
            if res.status_code == 200:
                ...
                self.root.after(0, lambda: self.status_lbl.configure(text=f"Brain: {active}"))
            else:
    ...
    def _api_trigger_brain(self):
        try:
            ...
            if res.status_code == 200:
                self.root.after(0, lambda: self.append_message("System", "Brain triggered successfully! Check main dashboard for outputs."))
            else:
            ...
            while True:
                try:
                    ...
                    if res.status_code == 200:
                        data = res.json()
                        active = data.get("active_brain", "DEVELOPMENT")
                        self.root.after(0, lambda active=active: self.status_lbl.configure(text=f"Brain: {active}"))
                except:
                    pass
                time.sleep(2)
