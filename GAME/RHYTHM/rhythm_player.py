...
# Import AIDriver from GENRE_FOLDERS/AI/CORE
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.append(os.path.join(BASE_DIR, "..", "..", "AI"))
sys.path.append(os.path.join(BASE_DIR, "..", "..", "AI", "CORE"))
from ai_driver import AIDriver

def main():
    print("🎵 Starting AI Rhythm Game Automator...")
    
    # 1. Open the game using Python's standard webbrowser module (highly robust)
    ...
    # 2. Connect to the window with robust retries
    print("🔌 Connecting to 'AI Rhythm' window...")
    driver = None
    for attempt in range(15):
        driver = AIDriver("AI Rhythm")
        if driver.hwnd:
            break
        print(f"⏳ Waiting for game window... (Attempt {attempt+1}/15)")
        time.sleep(1.0)
        
    if not driver or not driver.hwnd:
        print("❌ Could not connect to 'AI Rhythm' window.")
        return
